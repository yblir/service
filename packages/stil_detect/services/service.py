...
from services.handler import APIHandler
...
class Service(object):
    ...
    def run(self, port=8080, debug=False):
        ...
        self.pairs.append(
                tuple([self.url, APIHandler,
                       dict(data_type_keyword_dict=self.data_type_keyword, predictor=self.preditor)])
        )
    ...
    def register_list(self, params_list):
        # data_type_keyword_dict是tornado框架内部参数
        for url, predictor in params_list:
            self.pairs.append(tuple([url, APIHandler, dict(data_type_keyword_dict=self.data_type_keyword,
                                                           predictor=predictor)]))
