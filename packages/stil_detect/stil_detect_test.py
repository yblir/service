...
from modules.predict import STILPredictor
# 解码模块：图片微服务
...
if __name__ == "__main__":
    ...
    batch_predictor = STILPredictor(decoder, infer_module, "", data_type_keyword, "")
    ...
    frames = {}
    # frames2 = []
    # root_path = Path("/mnt/e/DeepFakeDetection/datasets/ff_frame/manipulated_sequences/Deepfakes/c23/videos/014_790")
    root_path = Path("/mnt/c/Users/Administrator/Desktop/test_imgs")
    root_path_img = [i for i in root_path.iterdir()]

    for i in range(15):
        if i >= 8:
            frames[i] = cv2.imread(str(root_path_img[8]))
        else:
            frames[i] = cv2.imread(str(root_path_img[i]))
    ...
    res, _, _, _, _ = batch_predictor.predict(request_data)
    pprint(res)
