...
class ImageInferModule(BaseModuleInfer):
    ...
    def __init__(self, params):
        ...
        if flag != 0:
            ...
            sys.exit()
        logger.success("init Engine success")
    ...
    def module_infer(self, input_data):
        ...
        # not isinstance(input_data, np.ndarray) and
        # 检查输入数据是否是图片矩阵
        if isinstance(input_data, list):
            for item in input_data:
                if not isinstance(item, np.ndarray):
                    ...
                    logger.error(
                            f"input data of inferEngine is list, "f"but have element is not np.ndarray, is {type(item)}"
                    )
            ...
            # 如果不是list和np.ndarray,打印error,提前结束推理任务
